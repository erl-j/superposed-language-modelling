...
def build_constraint(e, ec, n_events, beat_range, pitch_range, drums, tag, tempo):
                            ...
                            # Add hi-hats with tresillo rhythm (pitch 42)
                            for bar in range(4):
                                for beat in range(4):
                                    # First dotted eighth note
                                    e += [ec().intersect({"pitch": {"42 (Drums)"}, "onset/beat": {str(bar*4 + beat)}, "onset/tick": {"0"}}).force_active()]
                                    # Second dotted eighth note
                                    e += [ec().intersect({"pitch": {"42 (Drums)"}, "onset/beat": {str(bar*4 + beat)}, "onset/tick": {"12"}}).force_active()]
                                    # Third eighth note
                                    e += [ec().intersect({"pitch": {"42 (Drums)"}, "onset/beat": {str(bar*4 + beat)}, "onset/tick": {"18"}}).force_active()]
                            ...
                            # Set tag to 'electronic' and 'groove' as drill is not in the available tags
                            e = [ev.intersect({"tag": {"electronic", "groove"}}) for ev in e]
                            
                            # Add optional drum hits to fill out the beat
                            e += [ec().intersect({"instrument": {"Drums"}}) for _ in range(30)]
