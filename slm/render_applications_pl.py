...
from render_tasks import TASKS
...
OUTPUT_DIR = Path("./artefacts/applications_250h")
...
def main():
    ...
    parser.add_argument('--seed', type=int, default=0,
                      help='Random seed')
    parser.add_argument('--render-ground-truth', type=bool, required=True,
                      help='Should ground truth examples be rendered?')
    ...
    if args.render_ground_truth:
        ...
        if not list(reference_dir.glob("*.mid")):
            print("\nSaving ground truth examples...")
            for i, example in enumerate(tqdm(ground_truth_examples)):
                example_sm = first_model.tokenizer.decode(ground_truth_examples[i]["token_ids"])
                sm_set_track_order(example_sm).dump_midi(reference_dir / f"example_{i}.mid")
