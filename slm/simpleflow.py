...
class SimpleFlowModel(pl.LightningModule):
    ...
    def step(self, batch, batch_idx=None):
        ...
        p0 = torch.softmax(z0@self.U, dim=-1)
        p0 = p0 * self.format_mask[None,...].to(self.device)
        p0 = p0 / (p0.sum(dim=-1, keepdim=True)+1e-12)
        z0p = p0 @ self.E

        zt = t[:,None,None] * z1 + (1-t[:,None,None]) * z0p

        pt = torch.softmax(zt@self.U, dim=-1)
        # multiply with format mask
        # pt = pt * self.format_mask[None]
        pt = pt / (pt.sum(dim=-1, keepdim=True)+1e-12)
        assert not torch.isnan(pt).any()

        z1hat = self.forward(pt,t)

        assert not torch.isnan(z1hat).any()

        # return to probabilities
        p1hat = torch.softmax(z1hat@self.U,dim=-1)
        assert not torch.isnan(p1hat).any()
        # multiply with format mask 
        p1hat = p1hat * self.format_mask[None,...].to(self.device)
        assert not torch.isnan(p1hat).any()

        # renormalize
        p1hat = p1hat / (p1hat.sum(dim=-1,keepdim=True)+1e-12)
        assert not torch.isnan(p1hat).any()
        # rembed
        z1hat = p1hat @ self.E
        # get logits
