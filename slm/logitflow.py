...
class LogitFlowModel(pl.LightningModule):
    ...
    def sample(self, prior, n_steps, temperature):
        ...
        for step in tqdm(range(n_steps)):
            ...
            pts.append(pt.detach().cpu())

            vt = self.forward(pt,t)

            vts = vts + [vt.detach().cpu()]

            zt = zt + delta_t * vt

            zts = zts + [zt.detach().cpu()]
        ...
        pt_diff = pts[1:] - pts[:-1]
        plt.imshow(
            cmap_scale_fn(pt_diff[:,0,0,:].cpu().T),
            cmap="magma", aspect="auto",interpolation="none")
        plt.title("pt diff")
        plt.show()
