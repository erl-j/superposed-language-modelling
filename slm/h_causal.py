...
class HierarchicalCausalDecoderModel(pl.LightningModule):
    ...
    def event_forward(self, x):
        ...
        Args:
            x: (batch_size, events-1, attributes) tensor

        Returns:
            (batch_size, events-1, d) tensor
        '''
        ...
        ez = einops.repeat(self.event_embedding, "e d -> 1 e d", e = self.n_events)
        xz = xz + ez   
    ...
    def attribute_forward(self, x, event_z):
        ...
        logits = einops.rearrange(logits, "(b e) a d -> b e a d", a=self.n_attributes, e=self.n_events, b=x.size(0))
        return logits
    ...
    def step(self, batch, batch_idx):
        ...
        inputs = x
        targets = x
        inputs_ = einops.rearrange(inputs, "b (e a) -> b e a", e=self.n_events)
        logits = self(inputs_)
