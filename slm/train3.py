...
class SuperposedLanguageModel(torch.nn.Module):
    ...
    def generate(self,x):
        ...
        '''
        if self.use_mlm:
            return self.mlm_generate(x)
        else:
            return self.slm_generate(x)
    ...
    def slm_forward(self,x):
        ...
        syntax_mask = einops.repeat(syntax_mask, "1 1 a v -> b t a v", b=x.shape[0], t=x.shape[1])
        # apply syntax mask
        x = x * syntax_mask
        # renormalize
        ...
        logits = einops.repeat(logits, "b t v -> b t a v", a=len(self.tokenizer.note_attribute_order)).clone()
    ...
    def mlm_forward(self,x):
        ...
        Input:
            x: Input tensor of shape (batch_size, events, attributes, vocab_size+1)
        '''
        pass
