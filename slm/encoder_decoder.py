...
class DecoderOnlyModel(pl.LightningModule):
    def __init__(
        ...
        self.n_notes = self.tokenizer(self.tokenizer.config["max_notes"])

        self.note_decoder_positional_encoding  = nn.Parameter(torch.zeros(1, self.n_notes, hidden_size), requires_grad=True)
        self.embedding_layer = nn.Linear(vocab_size, hidden_size, bias=False)
        self.one_hot_input = one_hot_input
      
        self.note_decoder = torch.nn.TransformerDecoder(
            encoder_layer=torch.nn.TransformerDecoderLayer(
            d_model=hidden_size,
            nhead=n_heads,
        ...
        self.attribute_decoder = torch.nn.TransformerDecoder(
    ...
    def note_forward(self, con, tgt):
        ...
        ze = ze.sum(dim=2)
        pos = self.note_decoder_positional_encoding[:, :ze.shape[1], :].to(self.device)
        ze = ze + pos
    ...
    def forward(self, con, tgt):
        ...
        note_z = self.note_forward(con, tgt)
        decoder_logits = self.attribute_forward(con, tgt, note_z)
...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(log_model="all", project="slm-decoder")
    # get name
